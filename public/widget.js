;(function () {
  ...
  let greeting   = 'Chat with us'

  // ── Styles (injected after config fetch) ────────────────────────────────────
  ...
  function applyStyles() {
    styleEl.textContent = `
      ...
      #air-bubble:hover { filter: brightness(1.1); transform: scale(1.08); }
      ...
      #air-input:focus { border-color: ${brandColor}; }
      #air-send {
        width: 36px;
        height: 36px;
        border-radius: 50%;
      ...
      #air-send:disabled { opacity: 0.5; cursor: not-allowed; }
      #air-send:not(:disabled):hover { filter: brightness(1.1); }
  ...
  // ── DOM (built after config fetch) ──────────────────────────────────────────
  let bubble, win, messagesEl, inputEl, sendEl, form
  ...
  function buildDOM() {
    ...
    win.innerHTML = `
      <div id="air-header">
        <span id="air-greeting">${greeting}</span>
      ...
      <div id="air-powered">Powered by Nexus Forge AI</div>
    `

    document.body.appendChild(bubble)
    document.body.appendChild(win)

    messagesEl = win.querySelector('#air-messages')
    inputEl    = win.querySelector('#air-input')
    sendEl     = win.querySelector('#air-send')
    form       = win.querySelector('#air-form')
  ...
  // ── Boot: fetch config then build DOM ────────────────────────────────────────
  fetch(`${SERVER_URL}/api/widget-config?clientId=${CLIENT_ID}`)
    .then((r) => r.json())
    .then((cfg) => {
      brandColor = cfg.brand_color || '#4f46e5'
      greeting   = cfg.greeting    || 'Chat with us'
    })
    .catch(() => {})
    .finally(() => {
      applyStyles()
